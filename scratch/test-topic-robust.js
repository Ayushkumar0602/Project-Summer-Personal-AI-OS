function extractTopic(text) {
  const patterns = [
    ...
    /(?:research|report|study|paper|overview|summary)\s+(?:on|about|for|regarding|into)\s+['"]?([^"'.]+)['"]?/i,
    /(?:ppt|presentation|powerpoint|deck|slides?)\s+(?:on|about|for|regarding|into)\s+['"]?([^"'.]+)['"]?/i,
    /(?:make|create|build|generate|conduct)\s+(?:a\s+)?(?:ppt|presentation|powerpoint|deck|research|study|report|paper)\s+(?:on|about|for|regarding|into)\s+['"]?([^"'.]+)['"]?/i,
...
const prompts = [
  ...
  "Conduct deep research and produce a comprehensive research paper titled 'A Full Overview of AI'.",
  "I want you to do a deep research on the future of humanoid robots."
];
